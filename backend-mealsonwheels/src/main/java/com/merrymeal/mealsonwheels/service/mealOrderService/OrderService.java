...
public class OrderService {
    ...
    public List<OrderDTO> getOrdersForCurrentRider() {
        String username = SecurityContextHolder.getContext().getAuthentication().getName();
        RiderProfile rider = riderProfileRepository.findByUser_Username(username)
                .orElseThrow(() -> new ResourceNotFoundException("Rider not found"));
    ...
    private OrderDTO mapToOrderDTO(Order order) {
        return OrderDTO.builder()
                ...
                .memberUsername(order.getMember() != null ? order.getMember().getUser().getUsername() : null)
                .partnerId(order.getPartner() != null ? order.getPartner().getId() : null)
                .partnerName(order.getPartner() != null ? order.getPartner().getUser().getUsername() : null)
